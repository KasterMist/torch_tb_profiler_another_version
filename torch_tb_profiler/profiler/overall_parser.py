...
class OverallParser(object):
    class Costs:
        ...
        def create_from_statistics(cls, statistics: 'OverallParser.Statistics', total_duration: int):
            ...
            # print(len(statistics.cost_ranges[4]))
    ...
    class Statistics:
        ...
        def create_from_range(cls, steps: List[Tuple[int, int]], role_ranges: List[List[Tuple[int, int]]]):
            ...
            dataloader_sums = 0
            memcpy_sums = 0
            for i in role_ranges[4]:
                runtime_sums += i[1] - i[0]
            for j in role_ranges[5]:
                dataloader_sums += j[1] - j[0]
            for k in role_ranges[1]:
                memcpy_sums += k[1] - k[0]
            # print("runtime sums", runtime_sums)
            # print("dataloader sums", dataloader_sums)
            # print("memcpy sums", memcpy_sums)
            # print("len(role_ranges)", len(role_ranges))

            # runtime has the highest priority

            # runtime_range = role_ranges[4]
            # slots = merge_ranges(list(steps))
            # slots = subtract_ranges_lists(slots, runtime_range)
            #
            # for i, role in enumerate(role_ranges):
            ...
            #         # cost_ranges.insert(0, range) # wrong code
            #         slots = subtract_ranges_lists(slots, range)
            #
            #
            # # The last one is ProfileRole.Other
            # # cost_ranges.insert(0, slots) # wrong code
            # cost_ranges.append(slots)
            # cost_ranges.insert(4, runtime_range) # wrong code
    ...
    def aggregate(self, steps: List[Tuple[int, int]], role_ranges: List[List[Tuple[int, int]]]):
        ...
        for i, step in enumerate(steps):
            ...
            # print("self.steps_costs[0].costs[4]", self.steps_costs[0].costs[4])
            ...
            # print("comm_costs.overlap", comm_costs.overlap)
            if role_ranges[ProfileRole.Kernel]:
